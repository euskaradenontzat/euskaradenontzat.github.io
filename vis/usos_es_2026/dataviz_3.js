...
d3.tsv("data/municipios_data_3.tsv", function(error, zonas3) {
...
// rótulos secciones
	svg3.append("text")
		...
		.text("Menos de 5.001 habitantes")
		//.text(becasMaterial)
	...
	legend3.selectAll('div')
		...
		.on('click',function(d) { //when click on name
			...
			svg3.selectAll('svg .foreground path.'+ replacement2(d.zona))
				.style("opacity",1)
				.style("visibility","visible").attr("stroke-width","2px"); //selecciona path que coincide con la zona seleccionada
			d3.select(this).attr("class","btn-activo"); //adds class success to button
			if ( d.zona3 == "Todos") {
				zona3.select("p").html("<strong>Todos los municipios</strong>");
			} else {
				zona3.select("p").html("<strong>"+d.zona+"</strong>. <span style='font-size:0.7em'>"+ d.habitual_euskera_per + "% eusk. " + 
