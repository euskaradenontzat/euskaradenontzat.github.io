...
d3.tsv("data/municipios_data_2.tsv", function(error, zonas2) {
  ...
  foreground = svg2.append("g")
    ...
    .enter().append("path")
      .attr("d", path)
      .attr("class",function(d) { return replacement(d.zona) + " todos "+ d.provincia;} ) // colorea líneas según color de provincia
	...
	legend2.selectAll('div')
		...
		.on('click',function(d) { //when click on name
			...
			if ( d.zona2 == "Todos") {
				...
				zona2.select("p").html("<strong>"+d.zona+"</strong>. No hay centros privados en esta zona");
			} else {
				zona2.select("p").html("<strong>"+d.zona+"</strong>"); //write in description
			}
		});
});
